...
public abstract class SimpleNLG4Test extends TestCase {
    ...
    protected void setUp() {
        lexicon = new simplenlg.lexicon.catalan.XMLLexicon();  // built in lexicon

        this.phraseFactory = new NLGFactory(this.lexicon);
        this.realiser = new simplenlg.realiser.catalan.Realiser(this.lexicon);

        this.man = this.phraseFactory.createNounPhrase("el", "home"); //$NON-NLS-1$ //$NON-NLS-2$
        this.woman = this.phraseFactory.createNounPhrase("el", "dona");  //$NON-NLS-1$//$NON-NLS-2$
        this.dog = this.phraseFactory.createNounPhrase("el", "gos"); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        this.np6 = this.phraseFactory.createNounPhrase("el", "habitació"); //$NON-NLS-1$ //$NON-NLS-2$
        this.inTheRoom.addComplement(this.np6);

        this.underTheTable = this.phraseFactory.createPrepositionPhrase("sota"); //$NON-NLS-1$
        this.underTheTable.addComplement(this.phraseFactory.createNounPhrase("el", "taula")); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        this.getUp = this.phraseFactory.createVerbPhrase("aixecar"); //$NON-NLS-1$
        this.fallDown = this.phraseFactory.createVerbPhrase("caure"); //$NON-NLS-1$
        this.give = this.phraseFactory.createVerbPhrase("donar"); //$NON-NLS-1$
        this.say = this.phraseFactory.createVerbPhrase("dir"); //$NON-NLS-1$
    }
